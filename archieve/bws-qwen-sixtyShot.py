...
pipeline = IngestionPipeline(
    transformations=[
        SentenceSplitter(chunk_size=4000, chunk_overlap=60),
...
for idx, dev_item in enumerate(dev_items):
    ...
    # Retrieve top 60 similar train nodes for few-shot examples
...
with open('rag4re_predictions_60shot_rag_midSizeData.json', 'w') as out_f:
    json.dump(outputs, out_f, indent=2)

# Evaluation
wandb.init(project="relation-extraction", name="RAG4RE_60shot_RAG_completeData")
...
print(f"\nEvaluation RESULTS for 60-shot + RAG prompting:")
...
excel_filename = 'relation_extraction_results_60shot_rag_midSizeData.xlsx'
