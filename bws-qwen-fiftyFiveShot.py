...
pipeline = IngestionPipeline(
    transformations=[
        SentenceSplitter(chunk_size=4000, chunk_overlap=55),
...
for idx, dev_item in enumerate(dev_items):
    ...
    # Retrieve top 55 similar train nodes for few-shot examples
    retriever = index.as_retriever(similarity_top_k=55)
...
with open('rag4re_predictions_55shot_rag_midSizeData.json', 'w') as out_f:
    json.dump(outputs, out_f, indent=2)

# Evaluation
wandb.init(project="relation-extraction", name="RAG4RE_55shot_RAG_completeData")
...
print(f"\nEvaluation RESULTS for 55-shot + RAG prompting:")
...
excel_filename = 'relation_extraction_results_55shot_rag_midSizeData.xlsx'
