...
for idx, dev_item in enumerate(dev_items):
    ...
    messages = [
        ChatMessage(
            role="system",
            content=f"Find the relationship between two entities. Respond only with a valid JSON choosing one relation from this list: [{', '.join(relation_types)}]."
        ),
        ChatMessage(
            role="user",
            content=(
    ...
    # Append to outputs with both relation and reasoning
    outputs.append({
        "head": head_entity,
        "tail": tail_entity,
        ...
        "prediction": relation,
        "reasoning": reasoning,  # optionally store reasoning
        "ground_prediction": valid_relations.get((subject_label, object_label), 'None')
    })

    # Print both for inspection/logging
    print(f"Final prediction relation: '{relation}'")
    print(f"Reasoning:\n{reasoning}\n---")


# Save predictions
print("Saving predictions...")
with open('rag4re_bws_qwen_zero_shot.json', 'w') as out_f:
...
df.to_excel('results_bws-qwen-one-shot.xlsx', index=False)
